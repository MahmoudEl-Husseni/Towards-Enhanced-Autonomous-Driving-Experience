...
from pygame_client import Controller
import cv2
...
class World(object):
    ...
    def restart(self):
        ...
        self.depth_camera = CameraManager(self.player, self.hud, self._gamma)
        self.depth_camera.transform_index = cam_pos_index
        self.depth_camera.set_sensor(2, notify=False)


        self.ss_camera = CameraManager(self.player, self.hud, self._gamma)
        self.ss_camera.transform_index = cam_pos_index
        self.ss_camera.set_sensor(5, notify=False)
    ...
    def destroy_sensors(self):
        ...
        # ##########################
        self.depth_camera.sensor.destroy()
        self.depth_camera.sensor = None
        self.depth_camera.index = None

        self.ss_camera.sensor.destroy()
        self.ss_camera.sensor = None
        self.ss_camera.index = None
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if self._is_quit_shortcut(event.key):
                    ...
                    ###############################
                    world.depth_camera.toggle_camera()
                    world.ss_camera.toggle_camera()
                    world.lidar_sensor.toggle_camera()
                    ###############################
                    ...
                    ########################################
                    world.depth_camera.toggle_recording()
                    world.ss_camera.toggle_recording()
    ...
    def __init__(self, parent_actor, hud, gamma_correction):
        ...
        self.sensors = [
            ...
            ['sensor.lidar.ray_cast', None,
                'Lidar (Ray-Cast)', {'range': '50'}, {'points_per_second':'100'}],
...
def game_loop(args):
    ...
    try:
        ...
        # load preception models
        if TRAFFIC_SIGN_YOLO : 
            traffic_sign_yolo = ObjectDetection(weights_path=TRAFFIC_SIGN_YOLO_WEIGHTS, device=DEVICE)
        ...
        with CarlaSyncMode(world.world, 
            ...
            while True:
                ...
                snapshot, img_raw, depth_raw, ss_raw, imu_raw, gnss_raw = sync_mode.tick(timeout=2.0)
                ...
                # apply perception modules: 
                array = np.frombuffer(img_raw.raw_data, dtype=np.dtype("uint8"))
                array = np.reshape(array, (img_raw.height, img_raw.width, 4))
                array = array[:, :, :3].astype('uint8')
                
                if TRAFFIC_SIGN_YOLO:
                    traffic_signs_bboxes = traffic_sign_yolo.detect(array)

                    # denormalize traffic signs
                    traffic_signs_bboxes[:, [1, 3]] *= array.shape[0]
                    traffic_signs_bboxes[:, [2, 4]] *= array.shape[1]

                    print(traffic_signs_bboxes)
                    # print(traffic_signs_bboxes)
