...
def lidar_2_world_t(lidar_data, lidar_2_world) : 
    p_cloud_size = len(lidar_data)
    p_cloud = np.copy(np.frombuffer(lidar_data.raw_data, dtype=np.dtype('f4')))
    p_cloud = np.reshape(p_cloud, (p_cloud_size, LIDAR_CHANNELS))

    # Lidar intensity array of shape (p_cloud_size,) but, for now, let's
    # focus on the 3D points.
    if VERSION==0.9: 
        intensity = np.linalg.norm(p_cloud[:, :3], axis=1)
        intensity = np.exp(-LIDAR_ATTENUATION * intensity)
    else :
...
def world_2_image_t(data, world_2_camera, K): 
    # Transform the points from world space to camera space.
    sensor_points = np.dot(world_2_camera, data)
...
def tutorial(args):
    ...
    try:
        ...
        for frame in range(args.frames):
            ...
            world_points, intensity = lidar_2_world_t(lidar_data, lidar_2_world)

            # This (4, 4) matrix transforms the points from world to sensor coordinates.
            world_2_camera = np.array(camera.get_transform().get_inverse_matrix())

            points_2d = world_2_image_t(world_points, world_2_camera, K)
