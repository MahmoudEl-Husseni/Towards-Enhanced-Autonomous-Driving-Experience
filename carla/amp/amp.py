...
class AMP:
    ...
    def get_agent_vectors (self, object_vectors, candidate_density) -> np.ndarray: 
        '''
        Get the agent vectors

        args : 
            object_vectors : np.ndarray [n_objects, 6] : object vectors [xs, ys, xe, ye, ts, distance, angle, vx, vy, yaw, object_type]

        returns :
            agent_vectors : np.ndarray [n_objects, 8] : agent vectors [xs, ys, xe, ye, vx, vy, yaw, candidate_density]
        '''
        xs = object_vectors[:, 0]
        ys = object_vectors[:, 1]
        xe = object_vectors[:, 2,]
        ...
        cd = np.array([candidate_density]*len(xs))
    ...
    def get_amp_inputs (self, ) : 
        ...
        self.objects_vectors = self.object_tracker.vectorize_objects(object_ids, player_loc)

        for obj_idx in range(len(self.objects_vectors)) : 

            # get agent vectors 
            agent_vectors = self.get_agent_vectors(self.objects_vectors[obj_idx], len(self.objects_vectors))[None, ...]
